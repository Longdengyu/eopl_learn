...
module MyModuleDeclTypeLang where 
...
ident = do 
    ...
    guard (not (elem id ["let", "in", "if", "then", "else", "proc", 
                        ...
                        "interface", "body", "opaque", "transparent", 
                        "type"] ))
    return id 
...
namedTypeExpr :: Parser Type 
namedTypeExpr = do 
    name <- ident
    return $ NamedType name 

qualifiedTypeExpr :: Parser Type 
qualifiedTypeExpr = do 
    keyword "from"
    mName <- ident 
    keyword "take"
    tName <- ident 
    return $ QualifiedType mName tName
...
parseOpaqueTypeDecl :: Parser Decl 
parseOpaqueTypeDecl = do 
    keyword "opaque"
    name <- ident 
    return $ OpaqueTypeDecl name 

parseTransparentTypeDecl :: Parser Decl 
parseTransparentTypeDecl = do 
    keyword "transparent"
    name <- ident 
    keyword "="
    ty <- parseType
    return $ TransparentTypeDecl name ty

parseDefn :: Parser Defn 
parseDefn = try parseSimpleDefn 
    <|> parseTypeDefn

parseSimpleDefn :: Parser Defn 
parseSimpleDefn = do 
    varName <- ident
    keyword "="
    exp <- parseExpr
...
parseTypeDefn :: Parser Defn
parseTypeDefn = do 
    keyword "type"
    name <- ident 
    keyword "="
    ty <- parseType
    return $ TypeDefn name ty 
...
valueOfDefns defns env = case defns of 
    [] -> return $ env 
    ((Defn name bindExpr):rest) -> do 
        value <- valueOf bindExpr env 
        valueOfDefns rest (ExtendEnv name value env)
    ((TypeDefn name ty):rest) -> do 
        valueOfDefns rest env
...
--     1. OPAQUE-TYPES
--     2. Transparent Types
--         other name: type abbreviations, concrete type
-- TODO: add parse test