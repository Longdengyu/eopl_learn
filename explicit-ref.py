...
def tokenize(text):
    keywords = ['let', 'in', 'if', "then", "else", "zero?", "proc", "letrec", "newref", "deref", "setref", "begin", "end"]
    pos = 0
    while True:
...
class SetRefExp(Exp):
    def __init__(self, ref_exp, val_exp):
...
class Parser:
    ...
    def parse(self):
        ...
        elif token.type == "NEWREF":
            return self.parse_newref_exp()
        elif token.type == "DEREF":
            return self.parse_deref_exp()
        elif token.type == "SETREF":
            return self.parse_setref_exp()
    ...
    def parse_newref_exp(self):
        self.require("NEWREF")
        self.require("LEFT_P")
        exp = self.parse()
        self.require("RIGHT_P")
        return NewRefExp(exp)

    def parse_deref_exp(self):
        self.require("DEREF")
        self.require("LEFT_P")
        exp = self.parse()
        self.require("RIGHT_P")
